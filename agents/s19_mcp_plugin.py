        ...
        for tool in self._tools:
            ...
            agent_tools.append(
                _function_tool(
                    ...
                    tool.get("inputSchema", {"type": "object", "properties": {}}).get("properties", {}),
...
NATIVE_TOOLS = [
    _function_tool("bash", "Run a shell command.", {"command": {"type": "string"}}, ["command"]),
    _function_tool("read_file", "Read file contents.", {"path": {"type": "string"}}, ["path"]),
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        for call in assistant.tool_calls:
            ...
            tool_input = parsed_args or {}
            decision = None
            try:
                if parse_error:
                    output = parse_error
                else:
                    decision = permission_gate.check(call.function.name, tool_input)
                    if decision["behavior"] == "deny":
                        ...
                        output = handle_tool_call(call.function.name, tool_input)
            except Exception as e:
                output = f"Error: {e}"
            ...
            messages.append(
                {
                    ...
                    "content": normalize_tool_result(
                        call.function.name,
                        str(output),
                        decision.get("intent") if decision else None,
                    ),
                }
            )
    ...
    while True:
        ...
        if query.strip() == "/tools":
            for tool in build_tool_pool():
                ...
                prefix = "[MCP] " if tool_name.startswith("mcp__") else "       "
                print(f"  {prefix}{tool_name}: {tool['function'].get('description', '')[:60]}")
            continue
        if query.strip() == "/mcp":
