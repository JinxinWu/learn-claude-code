...
def agent_loop(messages: list, perms: PermissionManager):
    ...
    """
    while True:
        # One model turn: include the system instruction each call.
        # 中文注释：每一轮都把 SYSTEM 放在最前面，确保行为约束稳定。
        ...
        # Persist assistant output so later turns can reference prior reasoning.
        assistant_message = {"role": "assistant", "content": message.content or ""}
        if message.tool_calls:
            # Store tool call metadata in plain dict form for stable history serialization.
        ...
        if not message.tool_calls:
            # No tool request means the model is done for this user turn.
            return message.content or ""

        for tool_call in message.tool_calls:
            # 中文注释：先解析参数，再走权限判定（deny/ask/allow）。
            tool_name = tool_call.function.name
            try:
                # OpenAI tool arguments arrive as a JSON string.
                tool_input = json.loads(tool_call.function.arguments or "{}")
            except json.JSONDecodeError as exc:
                output = f"Error: invalid tool arguments: {exc}"
                print(f"  [INVALID ARGS] {tool_name}: {exc}")
                # Return parse errors as tool output so the model can self-correct.
                messages.append({
                    "role": "tool",
            ...
            # Every tool execution path (allow/deny/ask) is reflected back to the model.
            messages.append({
                "role": "tool",
                "tool_call_id": tool_call.id,
                "content": str(output),
...
if __name__ == "__main__":
    ...
    while True:
        ...
        response_text = agent_loop(history, perms)
        if response_text:
            print(response_text)
        print()