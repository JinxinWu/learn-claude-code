...
class SystemPromptBuilder:
    ...
    def _build_tool_listing(self) -> str:
        ...
        for tool in self.tools:
            ...
            name = fn.get("name", tool.get("name", "unknown")) if isinstance(fn, dict) else tool.get("name", "unknown")
            desc = fn.get("description", tool.get("description", "")) if isinstance(fn, dict) else tool.get("description", "")
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        for tool_call in message.tool_calls:
            ...
            try:
                # 中文注释：OpenAI 工具参数是 JSON 字符串，先解析再执行。
            ...
            # 中文注释：每次工具调用都必须回注 role=tool，驱动下一步推理。
            messages.append({
                "role": "tool",
                "tool_call_id": tool_call.id,
                "content": str(output),
