    ...
    while True:
        ...
        for tool_call in msg.tool_calls:
            ...
            try:
                ...
                try:
                    output = handler(**parsed_args) if handler else f"Unknown: {name}"
                except Exception as e:
                    output = f"Error: {e}"

            print(f"> {name}: {str(output)[:200]}")

            # 按 role=tool 回注输出，并绑定 tool_call_id。
            messages.append(
                {
                    "role": "tool",
